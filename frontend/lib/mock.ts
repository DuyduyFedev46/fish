...
export async function mockCreateOrder(payload: CreateOrderPayload): Promise<CreateOrderResponse> {
  ...
  for (const line of payload.items) {
    ...
    const lineTotal = Math.round(item.price * line.qty);
    total += lineTotal;
    lines.push({
...
export async function mockGetOrderStatus(
  ...
  return delay(toOrderStatus(resolved));
}

// Lập bộ tham số thanh toán cổng — giả lập BR-TT-01/13/14/17 (story P1, BE làm song song).
// Trang "cổng" trong mock chính là màn checkout của Shop với `?mock_gateway=1`
// (xem features/checkout/components/MockGatewayPanel.tsx), để không cần mạng thật.
export async function mockStartCheckoutSession(
  ...
  if (resolved.is_paid) {
    throw new ApiError("Đơn đã thanh toán.", 409);
  }
  if (resolved.is_expired || !resolved.booked_expires_at) {
    throw new ApiError("Đơn đã hết hạn giữ hàng, vui lòng đặt lại.", 409);
  }
  ...
  return delay({
    ...
    gateway_method: "GET",
    gateway_url: `${origin}/shop/checkout/`,
    fields: {
      mock_gateway: "1",
      order: orderCode,
      amount: String(resolved.total_amount),
      success_url: `${returnBase}&payment=success`,
      cancel_url: `${returnBase}&payment=cancel`,
      error_url: `${returnBase}&payment=error`,
    },
    environment: "SANDBOX",
  });
}
